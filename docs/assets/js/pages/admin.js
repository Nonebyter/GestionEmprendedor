...
document.getElementById('footer').textContent = `${BUSINESS.name} · Panel de administracion`;
...
$('login-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    showToast(err.code === 'auth/invalid-credential' ? 'Correo o contrasena incorrectos.' : err.message, 'danger');
  } finally {
    loading(false);
  }
});
...
async function cargarResumen() {
  ...
  $('tarjetas').innerHTML = cards.map(([t, v, icon, color]) => `
    <div class="col-6 col-lg-3"><div class="card shadow-sm h-100 border-0 border-start border-4 border-${color}">
      ...
      <div class="fs-4 fw-bold">${v}</div></div></div></div>`).join('');

  $('top-productos').innerHTML = s.topProducts.length
    ? `<ul class="list-group list-group-flush">${s.topProducts.map((p) => `
  ...
  $('stock-bajo').innerHTML = s.lowStock.length
    ? `<ul class="list-group list-group-flush">${s.lowStock.map((p) => `
        <li class="list-group-item d-flex justify-content-between px-0">
          ...
          <span class="badge text-bg-${Number(p.stock) <= 0 ? 'danger' : 'warning'}">${Number(p.stock) || 0} u.</span></li>`).join('')}</ul>`
...
async function cargarProductos() {
  ...
  const cats = [...new Set(productos.map((p) => p.category || 'General'))].sort();
...
async function cargarCompras() {
  ...
  $('tb-compras').innerHTML = compras.length ? compras.map((c) => `
    <tr data-key="${escapeHtml(`${c.product_name} ${c.supplier || ''} ${c.date}`)}">
      ...
      <td><div class="fw-semibold">${escapeHtml(c.product_name)}</div>
        ${c.supplier ? `<div class="small text-muted">${escapeHtml(c.supplier)}</div>` : ''}</td>
      <td class="text-center">${Number(c.quantity) || 0}</td>
      <td class="text-end fw-semibold">${money(c.total)}</td>
      <td class="text-end"><button class="btn btn-sm btn-outline-danger" data-del-compra="${c.id}"><i class="bi bi-trash"></i></button></td>
    </tr>`).join('')
...
async function cargarVentas() {
  ...
  $('tb-ventas').innerHTML = ventas.length ? ventas.map((v) => `
      ...
      <div class="card shadow-sm h-100 ${o.status === 'nuevo' ? 'border-danger' : ''}">
        <div class="card-body">
          ...
          <ul class="list-group list-group-flush mt-2">
            ${(o.items || []).map((i) => `<li class="list-group-item d-flex justify-content-between px-0 py-1 small">
              <span>${i.qty} x ${escapeHtml(i.name)}</span><span>${money(i.subtotal)}</span></li>`).join('')}
